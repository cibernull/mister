...
const cifrasDe = (e) => `<div class="cifras">
        <div><b class="caja${e.saldo < 0 ? ' baja' : ''}">${eur(e.saldo)}</b><i>en caja</i>${
          ...
          // cuando Mister dejaba gastar 5,4 M, y parecía un error nuestro.
          e.comprometido > 0 ? `<em class="baja" title="Está en tu caja, pero lo reservan las pujas que tienes puestas: no cuenta para lo que puedes gastar">−${eur(e.comprometido)} retenidos en pujas</em>` : ''
        }${
...
/**
 * Cuánto mejoraría tu mejor once si lo fichas: lo que se espera de él menos lo
 * del titular más flojo de su puesto, al que echaría. Se compara con el mejor
 * once equilibrado de hoy, el mismo que enseña la pestaña de inicio; y si esa
 * formación no tiene su puesto, no entra y no mejora nada.
 */
const mejoraDelOnce = (() => {
  const mejor = formacionesPorPerfil.equilibrado[0]?.once ?? once
  if (!mejor) return () => null
  const peorPorPuesto = new Map()
  for (const t of mejor.elegidos) {
    const da = noJuega(t) ? 0 : esperadoTotal(t)
    const p = peorPorPuesto.get(t.puesto)
    if (p === undefined || da < p) peorPorPuesto.set(t.puesto, da)
  }
  return (j) => {
    const peor = peorPorPuesto.get(j.puesto)
    if (peor === undefined || noJuega(j)) return null
    return esperadoTotal(j) - peor
  }
})()
...
/**
 * Lo que tiene que pasar para que algo sea una oportunidad de verdad.
 *
 * Antes cada objetivo dejaba pasar a cualquiera con la señal en positivo
 * —un euro de margen, un punto esperado, mil euros de subida— y así siempre
 * había cuatro tarjetas, hubiera o no algo que mereciera la pena. Se pidió lo
 * contrario: si no hay nada rentable, que no salga nada, o uno.
 *
 *   · Precio con margen: el techo sale de comparar con jugadores de media
 *     parecida a ±15 %, así que un margen menor que ese 15 % del techo está
 *     dentro del error del propio método y no es una ganga.
 *   · Ganar puntos: tiene que entrar en tu mejor once y mejorar en al menos
 *     un punto por jornada al titular que echaría. Quien no entra en el once
 *     no da puntos, dé la media que dé.
 *   · Generar dinero: que en un mes, si repite lo de este, devuelva al menos
 *     el 10 % de lo pagado. Por debajo, el vaivén diario del valor se lo come.
 *     Y que esté subiendo **ahora**: hoy y en los últimos siete días. Unai
 *     Simón salía con +10 % al mes y −31 K ese día: un tren que ya pasó no es
 *     una oportunidad de dinero.
 */
const MARGEN_MINIMO_ESCAPARATE = 0.15
const MEJORA_MINIMA_ESCAPARATE = 1
const RETORNO_MINIMO_ESCAPARATE = 0.1
const DIAS_DE_RACHA = 7

/**
 * Lo que ha hecho su valor en los últimos siete días, del histórico diario.
 * Null si no hay ocho fotos: sin recorrido no se afirma nada.
 */
const rachaDe = (j) => {
  const serie = SERIES.get(String(j.id))
  if (!serie || serie.valores.length <= DIAS_DE_RACHA) return null
  return (serie.valores.at(-1) - serie.valores.at(-1 - DIAS_DE_RACHA)) * 1000
}
const subeAhora = (j, racha) => (j.semana ?? 0) > 0 && racha !== null && racha > 0

/**
 * Los candidatos: solo libres, y solo los que se pueden pagar hoy.
 *
 * Un jugador con dueño no es una oportunidad tuya aunque esté en venta: la
 * puja la decide su dueño, que puede rechazarla, y quitárselo por la cláusula
 * cuesta como poco 1,5 veces lo que vale. Se pidió expresamente que no salgan,
 * «aunque los pongan en venta».
 */
const aptosEscaparate = J.filter((j) => {
  if (!j.a || j.mio || j.duenio) return false
...
}).map((j) => ({ j, renta: hastaCuanto(j), forma: formaDe(j), dinero: gananciaMesDe(j), mejora: mejoraDelOnce(j), racha: rachaDe(j) }))

const conMargen = (x) => x.renta !== null && x.renta.margen >= x.renta.techo * MARGEN_MINIMO_ESCAPARATE
const conPuntos = (x) => x.mejora !== null && x.mejora >= MEJORA_MINIMA_ESCAPARATE
const conDinero = (x) => x.dinero !== null && x.dinero >= x.j.precio * RETORNO_MINIMO_ESCAPARATE && subeAhora(x.j, x.racha)

const porMargen = aptosEscaparate.filter(conMargen).sort((a, b) => b.renta.margen - a.renta.margen || b.j.esperado - a.j.esperado)
const porPuntos = aptosEscaparate.filter(conPuntos).sort((a, b) => b.mejora - a.mejora)
const porDinero = aptosEscaparate.filter(conDinero).sort((a, b) => b.dinero - a.dinero)
...
const tarjetaOportunidad = (x, i) => {
  const { j, renta, forma, dinero, mejora, racha } = x
  const ahora = j.semana == null ? 'sin dato de hoy' : `hoy ${j.semana === 0 ? 'no se mueve' : firmaCorta(j.semana)}${racha === null ? '' : `, ${firmaCorta(racha)} en ${DIAS_DE_RACHA} días`}`
  ...
  return `      <article class="oportunidad" data-op-id="${j.id}" data-op-puntos="${mejora === null ? -1e9 : Math.round(mejora * 100) / 100}" data-op-valor="${dinero == null ? -1e12 : Math.round(dinero)}" data-apto-equilibrio="${conMargen(x) ? 1 : 0}" data-apto-puntos="${conPuntos(x) ? 1 : 0}" data-apto-valor="${conDinero(x) ? 1 : 0}" data-motivo-equilibrio="${esc(renta && renta.margen >= 0 ? `${corto(renta.margen)} por debajo de su techo estimado por rendimiento` : `${corto(-(renta?.margen ?? 0))} por encima de su techo estimado`)}" data-motivo-puntos="${esc(mejora !== null && mejora > 0 ? `Entra en tu mejor once: ${dec(j.esperado)} puntos esperados, ${dec(mejora)} más que el titular al que echa${probabilidadDe(j) != null ? ` · ${probabilidadDe(j)} % de salir` : ''}` : `No entra en tu mejor once: ${dec(j.esperado)} puntos esperados no mejoran a tu titular`)}" data-motivo-valor="${esc(dinero == null ? 'Sin dato de cómo se mueve su valor' : `${j.mk ? 'Pagas lo que vale' : `Pagas ${corto(j.precio)} de cláusula por ${corto(j.valor)} de valor`} · ${ahora}${subeAhora(j, racha) ? '' : ' · no está subiendo ahora'} · si repite lo de este mes, dentro de un mes ${dinero >= 0 ? 'ganas' : 'pierdes'} ${corto(Math.abs(dinero))}`)}" data-op-equilibrio="${Math.round(renta?.margen ?? -1e9)}">
...
const oportunidades = `    <section class="escaparate">
      <div class="escaparate-cab"><div><span class="eyebrow">Selección inteligente</span><h2>Oportunidades para ti</h2><p>Solo libres del mercado que puedes pagar hoy y que salen a cuenta de verdad. Si no hay ninguno, no sale ninguno.</p></div>${candidatosEscaparate.length ? `<span class="op-contador" id="op-cuenta">${Math.min(DE_GOLPE_ESCAPARATE, candidatosEscaparate.length)} de ${candidatosEscaparate.length}</span>` : ''}</div>
...
${candidatosEscaparate.map(tarjetaOportunidad).join(NL)}
      ...
      ${candidatosEscaparate.length > DE_GOLPE_ESCAPARATE ? `<button type="button" class="op-mas" id="op-mas">Ver ${DE_GOLPE_ESCAPARATE} más</button>` : ''}
      <p class="op-vacio sd" id="op-vacio"${candidatosEscaparate.length ? ' hidden' : ''}>${candidatosEscaparate.length ? 'Con este objetivo no hay ningún libre que salga a cuenta hoy.' : `Hoy no hay ningún libre en el mercado que salga a cuenta: ninguno tiene un ${Math.round(MARGEN_MINIMO_ESCAPARATE * 100)} % de margen sobre su techo, mejora tu once en un punto ni, subiendo hoy y esta semana, devolvería el ${Math.round(RETORNO_MINIMO_ESCAPARATE * 100)} % en un mes. Mejor no gastar que gastar por gastar.`}</p>
    </section>`
...
// La misma que encabeza el escaparate: un libre que pasa algún umbral. Si no
// hay ninguno, no se recomienda a nadie —antes salía el mejor de lo que hubiera,
// aunque fuera de otro equipo o no mereciera la pena—.
const oportunidad = candidatosEscaparate[0]?.j ?? null
    ...
    <summary>
      ...
      <div class="eqp"><b>${eur(e.tope)}</b><i title="${
        e.mio
          ...
          : 'Como mucho: si ahora mismo tiene alguna puja puesta, ese dinero le está retenido y no se puede ver —Mister solo publica lo retenido del equipo propio—. Se sabe cuando la puja se resuelve, y entonces la cuenta se corrige sola.'
      }">puede gastar${e.mio ? '' : ', como mucho'}</i></div>
      ${cifrasDe(e)}
    </summary>
    <div class="cuerpo">
...
const datoNoticia = (n, i) => {
  ...
  const relacionadas = jugador ? noticiasPriorizadas.filter((otra) => otra !== n && otra.jugadores.some((j) => String(j.id) === String(jugador.id))).length : 0
  const id = `n-${String(n.url || n.titulo).split('').reduce((a, c) => ((a * 31 + c.charCodeAt(0)) >>> 0), 7).toString(36)}`
  const attrs = `data-noticia-id="${id}" data-noticia-cat="${esc(n.categoria)}" data-noticia-mia="${n.mios.length ? '1' : '0'}" data-noticia-mercado="${n.mercado.length ? '1' : '0'}" data-noticia-url="${esc(n.url)}" data-noticia-titulo="${esc(n.titulo)}" data-noticia-fecha="${esc(n.fecha)}" data-noticia-relacion="${esc(relacionada)}" data-noticia-club="${esc(club || '')}" data-noticia-jugador="${jugador ? esc(jugador.nombre) : ''}" data-noticia-jugador-id="${jugador ? esc(jugador.id) : ''}" data-noticia-posicion="${jugador ? esc(PUESTOS_LARGO[jugador.puesto]) : ''}" data-noticia-media="${jugador?.media != null ? dec(jugador.media) : ''}" data-noticia-valor="${jugador?.valor != null ? esc(corto(jugador.valor)) : ''}" data-noticia-proximo="${jugador?.rival ? `${jugador.casa === 1 ? 'En casa' : 'Fuera'} contra ${esc(jugador.rival)}` : ''}" data-noticia-busca="${esc([n.titulo, n.categoria, relacionada, club, jugador?.nombre].filter(Boolean).join(' '))}"`
  const visual = `<div class="noticia-visual">${jugador ? caraDe(jugador.id, jugador.nombre) : `<span>${String(i + 1).padStart(2, '0')}</span>`}${logo}<i>${esc(n.categoria)}</i></div>`
  return { jugador, club, relacionadas, id, attrs, visual, relacionada }
}

const noticiaCard = (n, i) => {
  const d = datoNoticia(n, i)
  return `<article class="noticia-card${n.mios.length ? ' mia' : n.mercado.length ? ' mercado' : ''}" ${d.attrs} tabindex="0" aria-label="Abrir noticia: ${esc(n.titulo)}">${d.visual}<div class="noticia-contenido"><small>${esc(n.fecha)} · ${esc(d.relacionada)}</small><b>${esc(n.titulo)}</b><span>${d.club ? `${esc(d.club)} · ` : ''}FútbolFantasy${d.relacionadas ? ` · ${d.relacionadas} relacionada${d.relacionadas === 1 ? '' : 's'}` : ''}</span></div><button class="noticia-guardar" type="button" aria-label="Guardar noticia" title="Guardar noticia">☆</button><span class="noticia-nueva">Nueva</span></article>`
}

const noticiaDestacada = (n, i, principal = false) => {
  if (!n) return ''
  const d = datoNoticia(n, i)
  return `<article class="noticia-destacada${principal ? ' principal' : ''}${n.mios.length ? ' mia' : ''}" ${d.attrs} tabindex="0" aria-label="Abrir noticia: ${esc(n.titulo)}">${d.visual}<div class="noticia-destacada-cuerpo"><span>${n.mios.length ? 'Prioridad · Tu equipo' : n.mercado.length ? 'Oportunidad · Mercado' : esc(n.categoria)}</span><h3>${esc(n.titulo)}</h3><p>${esc(n.fecha)} · ${esc(d.relacionada)}${d.relacionadas ? ` · ${d.relacionadas} noticias relacionadas` : ''}</p><b>Ver contexto <i>→</i></b></div><button class="noticia-guardar" type="button" aria-label="Guardar noticia">☆</button></article>`
}

const noticiasMias = noticiasPriorizadas.filter((n) => n.mios.length).length
const noticiasMercado = noticiasPriorizadas.filter((n) => n.mercado.length).length
const noticiasCriticas = noticiasPriorizadas.filter((n) => ['lesión', 'sanción', 'convocatoria'].includes(n.categoria)).length
const portada = noticiasPriorizadas.slice(0, 3)

const noticiasHtml = `    <section class="actualidad" aria-labelledby="actualidad-titulo">
      <header class="actualidad-cab"><div><span class="eyebrow">Redacción · Inteligencia Fantasy</span><h2 id="actualidad-titulo">Lo que puede cambiar tu jornada</h2><p>Actualidad ordenada por relevancia para tu plantilla, el mercado y tus próximas decisiones.</p></div><a href="${esc(NOTICIAS_FANTASY.url || 'https://www.futbolfantasy.com/laliga/noticias')}" target="_blank" rel="noopener">Fuente original <span>↗</span></a></header>
      <div class="noticias-portada"><div class="noticias-destacadas">${noticiaDestacada(portada[0], 0, true)}<div>${noticiaDestacada(portada[1], 1)}${noticiaDestacada(portada[2], 2)}</div></div><aside class="noticias-impacto"><span class="eyebrow">Radar personal</span><h3>Tu impacto hoy</h3><div><button type="button" data-noticia-filtro="mias"><b>${noticiasMias}</b><span>sobre tu equipo</span><i>→</i></button><button type="button" data-noticia-filtro="mercado"><b>${noticiasMercado}</b><span>del mercado</span><i>→</i></button><button type="button" data-noticia-filtro="criticas"><b>${noticiasCriticas}</b><span>alertas deportivas</span><i>→</i></button></div><p><i></i>Actualizado ${haceFuente(fechaFuente('noticias-fantasy.json', NOTICIAS_FANTASY.cuando))}</p></aside></div>
      <div class="noticias-barra"><label class="noticias-buscar"><span>⌕</span><input id="noticias-busqueda" type="search" aria-label="Buscar noticias" placeholder="Buscar jugador, club o tema…" autocomplete="off"></label><div class="noticias-filtros" aria-label="Filtrar noticias"><button type="button" class="on" data-noticia-filtro="todas">Todas</button><button type="button" data-noticia-filtro="noleidas">No leídas</button><button type="button" data-noticia-filtro="mias">Mi equipo</button><button type="button" data-noticia-filtro="mercado">Mercado</button><button type="button" data-noticia-filtro="lesión">Lesiones</button><button type="button" data-noticia-filtro="sanción">Sanciones</button><button type="button" data-noticia-filtro="entrenamiento">Entrenos</button><button type="button" data-noticia-filtro="convocatoria">Convocatorias</button><button type="button" data-noticia-filtro="seguidas">Siguiendo</button><button type="button" data-noticia-filtro="guardadas">Guardadas</button><button type="button" data-noticia-filtro="silenciadas">Silenciadas</button></div><button class="noticias-densidad" id="noticias-densidad" type="button" title="Cambiar densidad"><span>☷</span> Compacta</button></div>
      <div class="noticias-lista-cab"><div><span class="eyebrow">Última hora</span><h3>Noticias para decidir mejor</h3></div><span id="noticias-cuenta">${noticiasPriorizadas.length} noticias</span></div>
      <div class="noticias-grid">${noticiasPriorizadas.map(noticiaCard).join('')}</div><button class="noticias-mas" id="noticias-mas" type="button">Mostrar más noticias <span>↓</span></button><div class="noticias-vacio" id="noticias-vacio" hidden><b>No hay noticias con estos filtros</b><span>Prueba otra categoría o borra la búsqueda.</span></div>
      ...
      <div class="noticia-panel-fondo" id="noticia-panel-fondo" hidden></div><aside class="noticia-panel" id="noticia-panel" hidden aria-labelledby="noticia-panel-titulo"><button class="noticia-panel-cerrar" type="button" aria-label="Cerrar">×</button><div class="noticia-panel-visual" id="noticia-panel-visual"></div><div class="noticia-panel-cuerpo"><span class="eyebrow" id="noticia-panel-meta"></span><h3 id="noticia-panel-titulo"></h3><div class="noticia-panel-jugador" id="noticia-panel-jugador"></div><div class="noticia-panel-acciones"><button type="button" id="noticia-panel-guardar">☆ Guardar</button><button type="button" id="noticia-panel-seguir" hidden>＋ Seguir jugador</button><button type="button" id="noticia-panel-silenciar" hidden>Silenciar</button><button type="button" id="noticia-panel-ficha" hidden>Ver ficha</button><a id="noticia-panel-enlace" target="_blank" rel="noopener">Leer en FútbolFantasy ↗</a></div><p>Mostramos el titular original y contexto calculado con los datos de tu liga. La información completa se consulta en la fuente.</p></div></aside>
    </section>`
...
<b>tope de puja</b> = caja + 25 % del valor de tu plantilla
    ...
    <div class="tarjeta">
      ...
      <div class="defs">
        ${def('★', 'txt', 'Oportunidades para ti', `Solo <strong>libres</strong> del mercado: un jugador con dueño no es una oportunidad tuya aunque esté en venta —la puja la decide su dueño, y quitárselo por la cláusula cuesta como poco 1,5 veces lo que vale—. Y solo si pasa algún umbral: <strong>margen</strong> de al menos el ${Math.round(MARGEN_MINIMO_ESCAPARATE * 100)} % sobre su techo (por debajo está dentro del error del propio cálculo), <strong>puntos</strong> si entra en tu mejor once y mejora en ${MEJORA_MINIMA_ESCAPARATE} punto al titular que echa, o <strong>dinero</strong> si en un mes, repitiendo lo de este, devuelve el ${Math.round(RETORNO_MINIMO_ESCAPARATE * 100)} % de lo pagado <strong>y está subiendo ahora</strong>, hoy y en los últimos ${DIAS_DE_RACHA} días: un +10 % al mes con el valor cayendo hoy es un tren que ya pasó. Si hoy no lo pasa nadie, no sale nadie.`)}
